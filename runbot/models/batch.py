...
class Batch(models.Model):
    ...
    def _compute_all_build_ids(self):
        for batch in self:
            # we expect to have a low number of batch, so we accept the search for caching purposes
            batch.all_build_ids = self.env['runbot.build'].search([('id', 'child_of', batch.slot_ids.build_id.ids)])
...
class BatchSlot(models.Model):
    ...
    def _compute_all_build_ids(self):
        for slot in self:
            # we expect to have a low number of batch, so we accept the search for caching purposes
            slot.all_build_ids = self.env['runbot.build'].search([('id', 'child_of', slot.build_id.ids)])
